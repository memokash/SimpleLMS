...
/**
 * Get courses with user progress - MAIN FUNCTION (renamed for dashboard compatibility)
 */
export async function getAllCoursesWithProgress(userId) {
  try {
    const courses = await getAllCourses();
...
export async function getCoursesWithProgress(userId) {
  return getAllCoursesWithProgress(userId);
}
